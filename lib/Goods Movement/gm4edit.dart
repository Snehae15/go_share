...
class gm4edit extends StatefulWidget {
  gm4edit({Key? key,required this.starting_point,required this.destination,required this.time,required this.date,required this.vehicle_no,required this.gm_id}) : super(key: key);
  var starting_point,destination,time,date,vehicle_no,gm_id;
  @override
...
class _gm4editState extends State<gm4edit> {
  ...
  Future<void> senddata()  async {
    ...
    var response=await post(Uri.parse("${con.url}goods_movement/update.php"),body: data);
    print(response.body);
    if (response.statusCode==200) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Updated')));
      Navigator.pushAndRemoveUntil(context,MaterialPageRoute(builder: (context)=>gm4()),(Route <dynamic> route )=>false);
    }
